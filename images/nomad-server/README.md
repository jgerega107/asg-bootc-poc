...
`retry_join` expression in `/etc/consul.d/90-runtime.hcl`. Servers do not
receive a Vault address. Nomad and its Consul client are ordered after
`cloud-config.service` and require their runtime HCL files.
