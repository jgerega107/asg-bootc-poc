...
variable "name" {
  description = "Name for the Consul server ASG, instances, and IAM resources."
  type        = string
...
variable "instance_type" {
  description = "EC2 instance type for Consul server instances."
  type        = string
  default     = "t3.micro"
}

variable "root_disk_size" {
  description = "Root disk size in GiB. Consul keeps its data on this disk."
  type        = number
  default     = 20
...
variable "root_disk_encrypted" {
  description = "Whether to encrypt the Consul root disk with the default EBS key."
  type        = bool
  default     = true
}
...
variable "vpc_security_group_ids" {
  description = "Additional security groups to attach to Consul server instances."
  type        = list(string)
  default     = []
}

variable "client_cidr_blocks" {
  description = "Additional CIDR blocks allowed to reach Consul client gossip and DNS ports. The private subnet CIDR is always allowed."
  type        = list(string)
  default     = []

  validation {
    condition     = alltrue([for cidr in var.client_cidr_blocks : can(cidrhost(cidr, 0))])
    error_message = "client_cidr_blocks must contain valid CIDR blocks."
  }
}

variable "bootstrap_expect" {
  description = "Number of Consul servers expected to form the initial cluster."
  type        = number
  default     = 1

  validation {
    condition     = var.bootstrap_expect >= 1 && floor(var.bootstrap_expect) == var.bootstrap_expect
    error_message = "bootstrap_expect must be a positive whole number."
  }
}

variable "min_size" {
  description = "Minimum number of Consul server instances in the Auto Scaling Group."
  type        = number
  default     = 1
...
variable "desired_capacity" {
  description = "Desired number of Consul server instances in the Auto Scaling Group."
  type        = number
  default     = 1
...
variable "max_size" {
  description = "Maximum number of Consul server instances in the Auto Scaling Group."
  type        = number
  default     = 1
...
variable "tags" {
  description = "Additional tags for Consul resources."
  type        = map(string)
  default     = {}
}
